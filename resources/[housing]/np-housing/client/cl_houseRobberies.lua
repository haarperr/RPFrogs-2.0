...
function attemptToLockPickHouse(skipPicking)
    ...
    if not canRobProperty() then
        TriggerEvent("DoLongHudText", "I don't think it's a good choice to mess around here this early", 2)
        return
    end
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["np-taskbar"]:taskBar(math.random(5000, 10000), "breaking in")
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["np-interaction"]:showInteraction("[E] Take Object")
        else
            exports["np-interaction"]:showInteraction("[E] Searching...")
        end
    end
end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("np-heists:complete", math.random(30, 50))
...
AddEventHandler("np-housing:alarmEnter", function()
    ...
    for i = 1, 6 do
        ...
        local finished = exports["np-taskbarskill"]:taskBarSkill(unitygordolamamaco[1],  unitygordolamamaco[2])
        if finished ~= 100 then
            failedAlarm = true
            break
        end
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("np-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("np-housing:alarm")
AddEventHandler("np-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["np-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("np-housing:beingRobbed", function(pPropertyId)
