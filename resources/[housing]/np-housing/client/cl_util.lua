...
function canRent()
    ...
    if isComplete and dist <= 3.0 then
        ...
        local canRent = RPC.execute("np-housing:canRent", propertyId)
...
function hasKey(propertyID)
    ...
    if exports["np-jobs"]:getJob(job, "is_police") or job == "judge" then
        return true
    end
...
AddEventHandler("property:menuAction", function(pData)
    ...
    if isComplete and dist <= 3.0 then
        ...
        elseif action == "forfeit" then
            ...
            exports["np-context"]:showContextMenu(MenuData["property_check"])
        end
    else
...
AddEventHandler("np-housing:handler", function(data, cb)
