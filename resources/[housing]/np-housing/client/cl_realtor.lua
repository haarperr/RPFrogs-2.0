            ...
            if #(pos.backdoor_coordinates.external-player) <= Housing.ranges.doorEnterRange and max.canHaveBackDoor then
                if not isLocked(propertyID) or Housing.currentOwned[propertyID] ~= nil or Housing.currentKeys[propertyId] ~= nil then
                    if lockdownCheck(propertyID) then
                        ...
                        TriggerEvent("DoLongHudText","Property bloqueada, voc� n�o pode acessar.",2)
                    end
                end
        ...
        if #(vec3FromVec4(Housing.info[propertyID]["pos"])-player) <= Housing.ranges.doorEnterRange then
            if not isGov() then
                if Housing.currentHousingLocks == nil or Housing.currentHousingLocks[propertyID] == nil then
                    TriggerEvent("DoLongHudText","Property Blocked - No Access",2)
                    return
                end
            end
...
function canPlaceAtLocation(pos,propertyID)
    if Housing.currentlyInsideBuilding then
        return false, "Cannot edit this inside the property"
    end

    if not playerInRangeOfProperty(propertyID) then
        return false, "Out of property range"
    end
    ...
    if not cast.Hit then
        return false, "can't be floating"
    end
    ...
    if difference > 15 then
        return false, "Too far from door height"
    end
...
function enterEdit(pPropertyId)
    ...
    if playerInRangeOfProperty(propertyID) then
        ...
        if Housing.currentHousingLockdown ~= nil and Housing.currentHousingLockdown[propertyID] and isResult == false then
            TriggerEvent("DoLongHudText", "Cannot edit a locked property.", 2)
            return
        end
...
function setInventory()
    ...
    if Housing.currentOwned[Housing.currentlyEditing] == nil then
        TriggerEvent("DoLongHudText", "you don't own the property", 2)
        return
    end
    ...
    if Housing.currentlyInsideBuilding then
        ...
        TriggerEvent("DoLongHudText", "off the property", 2)
    end
end
...
function placeBench(dropInventory)
    ...
    if objFound and objFound ~= 0 then
        if Housing.currentlyInsideBuilding then
            ...
            local finished = RPC.execute("updateCurrentSelected",Housing.currentlyEditing,Housing.currentHousingInteractions,Housing.hasEditedOrigin)
            if finished then
                TriggerEvent("DoLongHudText", "altered craft")
            end
        else
            TriggerEvent("DoLongHudText", "off the property", 2)
        end
...
function setBackdoor()
    ...
    if not hasCorrectFlags("canHaveBackDoor",Housing.currentlyEditing) then
        TriggerEvent("DoLongHudText", "Not available for the property.", 2)
        return
    end
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("np-editor:destroyName", id)
    end
...
function isGov()
    ...
    if exports["np-jobs"]:getJob(myjob, "is_police") or myjob == "judge" then
        return true
    end
    return false
end
...
function canPlaceInteractionPoint(nameIncoming,pos)
    ...
    if not canPlace then
        TriggerEvent("DoLongHudText", "It is not possible to place the point here, it may be too close to another point.", 2)
    end
...
function lockdownCheck(propertyID)
    ...
    if Housing.currentHousingLockdown[propertyID] and (exports["np-jobs"]:getJob(myjob, "is_police") or myjob == "judge") then
        return true
    end
...
AddEventHandler("np-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local tax = RPC.execute("np-financials:priceWithTax", Housing.info[propertyId]["price"], "Propertys")

    exports["np-context"]:showContext({
        {
            title = "Property",
        ...
        {
            title = "Price",
            description = "$" .. tax.total .. " Incl. " .. tax.porcentage .. "% in taxes",
        },
        {
            title = "Sell",
            action = "np-housing:sellTo",
            params = { propertyId = propertyId },
        }
    })
end)

AddEventHandler("np-housing:sellTo", function(pParams)
	...
	if input["cid"] then
		...
		TriggerServerEvent("np-housing:sell", cid, pParams.propertyId)
	end
end)

RegisterNetEvent("np-housing:buy")
AddEventHandler("np-housing:buy", function(pPropertyId, pSeller)
    local tax = RPC.execute("np-financials:priceWithTax", Housing.info[pPropertyId]["price"], "Propertys")
...
AddEventHandler("np-housing:rent", function()
    ...
    exports["np-context"]:showContext({
        ...
        {
            title = "Rent",
            action = "np-housing:rent2",
            params = { propertyId = propertyId, total = tax.total, tax = tax.tax },
        }
    })
end)

AddEventHandler("np-housing:rent2", function(pParams)
    local complete, info = RPC.execute("np-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
AddEventHandler("np-housing:buyed", function(pParams)
    local complete, info = RPC.execute("np-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
AddEventHandler("np-housing:edit", function()
    local context = {}
...
AddEventHandler("np-housing:editOptions", function(pParams)
...
AddEventHandler("housing:inventory", function()
    ...
    if max.canHaveInventory then
        if not lockdownCheck(propertyID) then
            TriggerEvent("DoLongHudText", "Property locked, you can't open inventory", 2)
            return
        end
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["np-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["np-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("np-housing:blips")
AddEventHandler("np-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = RPC.execute("np-housing:owned")
