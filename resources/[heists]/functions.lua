...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (not IsPedInAnyVehicle(player)) then
        ...
        if (itemid == "DuffelBag") then
            local finished = exports["np-taskbar"]:taskBar(10000,"Opening Duffel Bag",false,false,playerVeh)
        ...
        if (itemid == "MedicalBag") then
            local finished = exports["np-taskbar"]:taskBar(10000,"Opening Medical Supplies",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "pdbadge") then
        local ItemInfo = GetItemInfo(slot)
        TriggerServerEvent("np-badge:server:useitem", ItemInfo.information)
    end
    ...
    if (itemid == "weedoz" and hasEnoughOfItem("qualityscales",1,false) ) then
      ...
      local finished = exports["np-taskbar"]:taskBar(5000,"Rolling Joints",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint" or itemid == "joint2") then
        ...
        if (finished == 100) then
            ...
            if itemid == "joint" then
                TriggerEvent("np-hud:updateStress",false,50)
            end
            ...
            TriggerEvent("np-hud:updateStress",false,20)
            TriggerEvent(
              "weed",
              5000,
    ...
    if (itemid == "safecrackingkit") then
        TriggerEvent('np-robberies:HitSafes')
        local myJob = exports["isPed"]:isPed("myJob")
        if myJob ~= "news" then
    ...
    if (itemid == "heistlaptop3") then
        TriggerEvent('np-fleeca:UseGreenLapTop')
    end 
    ...
    if (itemid == "pixellaptop") then
        local finished = exports["np-taskbar"]:taskBar(3500,"Powering UP",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "durpman") then
        local finished = exports["np-taskbar"]:taskBar(1500,"Opening",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "disabler") then
        local finished = exports["np-taskbar"]:taskBar(500,"Powering UP",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (has_value(fruits, itemid)) then
        ...
        TriggerEvent("np-hud:ChangeHunger", 40)
    end
    ...
    if (itemid == "shiningpack") then ---------------- DONE 
        TriggerEvent('np-pokemon:openPack', itemid)
    end
    
    if (itemid == "bulbasaur") then ---------------- DONE 
        local ItemInfo = GetItemInfo(slot)
        TriggerEvent("np-pokemon:previewCard", itemid, ItemInfo.information)
    end  
        ...
        if itemInfo == nil then
            ...
            if itemInfo ~= nil then
                if itemInfo.id ~= nil then
                    ...
                    TriggerEvent("np-tcg:open-binder")
                end
            end
        end
    end
    ...
    if itemid == "tattooremover" then
        local target = exports['np-polytarget']:GetCurrentEntity()
    ...
    if itemid == "buddhaletter" then
        local finished = exports["np-taskbar"]:taskBar(5000,"Opening Parcel.",false,false,playerVeh)
        if finished == 100 then
        ...
        if hasEnoughOfItem("buddhablade",1,false) and hasEnoughOfItem("buddhaguard",1,false) and hasEnoughOfItem("buddhahilt",1,false) then
            if hasEnoughOfItem("buddhamedalion",1,false) then
                ...
                if #(playerPos - vector3(-453.333, 1.6416, 35.5193)) < 3 then
                    local finished = exports["np-taskbar"]:taskBar(30000,"Putting blade together.",false,false,playerVeh)
                    if (finished == 100) then
...
AddEventHandler('inv:wellfed', function()
    ...
    TriggerEvent("np-hud:updateStress",false,30)
    TriggerEvent("np-hud:ChangeHunger", 60)
    TriggerEvent("np-hud:ChangeThirst", 60)
    burgies = 0
end)
...
AddEventHandler('inv:fed2', function()
    ...
    TriggerEvent("np-hud:ChangeThirst", 70)
    processStressBlock()
    burgies3 = 0
end)
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["np-bar"]:taskBar(25000,3)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            TriggerEvent("civilian:alertPolice",12.0,"lockpick",targetVehicle)
            local finished = exports["np-bar"]:taskBar(5000,math.random(5,10))
...
AddEventHandler('inv:lockPick2', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["np-taskbar"]:taskBar(5000,"Picking")
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["np-bar"]:taskBar(5000,math.random(4,8))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        exports["np-taskbar"]:taskBar(15000, "Fixing Vehicle")
                        if finished == 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
