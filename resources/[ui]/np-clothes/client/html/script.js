...
$(function () {
    ...
    function CloseMenu(save, paymentType) {
        $.post('https://np-clothes/escape', JSON.stringify({
            save:save,
            paymentType,
    ...
    $(document).on('contextmenu', function() {
        $.post('https://np-clothes/togglecursor', JSON.stringify({}));
    })
    ...
    $('.button-applytattoo').on('click', function () {
        $.post('https://np-clothes/applytattoo', JSON.stringify({
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('https://np-clothes/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothing_shop')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if ($(ele).parents('.panel').attr('id') == "skins") {
                $.post('https://np-clothes/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('https://np-clothes/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('https://np-clothes/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothes/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothes/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothes/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothes/switchcam', JSON.stringify({name: 'cam'}))
    })

    $('.backup_save').on('click', function() {
        $.post('https://np-clothes/getPlayerFace')
    })
    ...
    $('.backup_load').on('click', function() {
        $.post('https://np-clothes/setPlayerFace',$("#backupJsonInput").val())
    })

    $('.tog_hat').on('click', function() {
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('https://np-clothes/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('https://np-clothes/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('https://np-clothes/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('https://np-clothes/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('https://np-clothes/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('https://np-clothes/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('https://np-clothes/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('https://np-clothes/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
