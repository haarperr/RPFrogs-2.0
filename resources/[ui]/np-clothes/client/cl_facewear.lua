...
function toggleFaceWear(pType, pRemove, pInfo, pSteal)
    ...
    if not pRemove then
        if pInfo.gender == "male" and not IsMale then
            TriggerEvent("DoLongHudText", "This outfit doesnt fit you. ", 2)
            return
        end
    end
    ...
    TriggerEvent("np-clothes:saveCurrentClothes")
...
AddEventHandler("np-inventory:itemUsed", function(item, info)
    ...
    if removing then
        TriggerEvent("DoLongHudText", "ok slower?", 2)
        return
    end
...
AddEventHandler("np-facewear:steal", function(pArgs)
...
AddEventHandler("np-facewear:radial", function(pArgs)
    if antispam >= GetCloudTimeAsInt() then
        TriggerEvent("DoLongHudText", "ok slower?", 2)
        return
    end
...
AddEventHandler("np-facewear:clothesMenu", function(pArgs, pEntity)
    local context = {}
    local text = "Remove"
    local event = "np-facewear:radial"

    if pArgs == "steal" then
        text = "Steal"
        event = "np-facewear:steal"
    else
        pEntity = PlayerPedId()
    end
