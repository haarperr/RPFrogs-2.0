$(document).ready(function () {
    ...
    function closeGui() {
        ...
        $.post("https://np-radio/close", JSON.stringify({}));
    }
    ...
    function Save() {
        if (Powered) {
            ...
            $.post("https://np-radio/setRadioChannel", JSON.stringify({
    ...
    $("#power").click(function () {
        if (Powered === false) {
            ...
            $("#RadioChannel").attr("placeholder", "100.0+");
            $.post("https://np-radio/poweredOn", JSON.stringify({
                channel: RadioChannel
            }));
        } else {
            Powered = false;
            $.post("https://np-radio/poweredOff", JSON.stringify({}));
            $("#RadioChannel").val("");
            $("#RadioChannel").attr("placeholder", "Off");
            $("#RadioChannel").prop("disabled", true);
        }
    ...
    $("#volumeUp").click(function () {
        $.post("https://np-radio/volumeUp", JSON.stringify({}));
    });

    $("#volumeDown").click(function () {
        $.post("https://np-radio/volumeDown", JSON.stringify({}));
    });
