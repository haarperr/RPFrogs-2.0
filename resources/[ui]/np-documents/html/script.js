...
function Form(_title, _subtitle, _elements, _submittable) {
	...
	this.sign = function () {
		...
		$("#signature_block").html(activeform.headerFirstName + " " + activeform.headerLastName + "<br>" + getCreationDate());
	}
	...
	this.submit = function () {
		...
		if (!this.signed) {
			can_submit = false;
			$("#signature_block").addClass("must_fill");
		}

		if (can_submit) {
			$.post(`https://${GetParentResourceName()}/form_submit`, json_string);
			activeform.close();
		}
	...
	this.print = function () {
		...
		html_header += "<h4>" + this.headerJobLabel + " </h4>";
		html_header += "</div>";
		html_header += "</div>";
		html_header += "</div>";
	...
	this.close = function () {
		...
		$.post(`https://${GetParentResourceName()}/form_close`, JSON.stringify({}));
	}
}
