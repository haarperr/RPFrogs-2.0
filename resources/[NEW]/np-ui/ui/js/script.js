...
$(function() {
    sleep(5000,function(){
        $.post('https://np-ui/np-ui:getKVPValue', JSON.stringify({
            key : "np-preferences",
        }),function(result){
            ...
            $.post('https://np-ui/np-ui:hudSetPreferences', JSON.stringify(uisettings),function(result){
...
document.querySelector('#setting_hud_save').addEventListener('click', function(){
    ...
    $.post('https://np-ui/np-ui:setKVPValue', JSON.stringify({
        key : "np-preferences",
        value:uisettings
    }),function(res){
...
function reset(checked){
    ...
    if (checked) {
        ...
        //     $.post('https://np-ui/'+minigame_memory_endpoint, JSON.stringify({
        //         result : true
        //     }));
        // })
    }else{
...
function checkanswernumbers(){
    ...
    if(game_started_numbers && response === answer_numbers.join('')){
        ...
        sleep(3000, function(){
            ...
            $.post('https://np-ui/'+gameFinishedEndpointNumbers, JSON.stringify({
                success : true
            }));
            $.post('https://np-ui/np-ui:closeApp', JSON.stringify({}),function(res){
...
document.querySelector('#textbox_submit').addEventListener('click', function(){
    ...
    $.post('https://np-ui/np-ui:closeApp', JSON.stringify({}),function(res){
        $.post('https://np-ui/'+textboxcallback, JSON.stringify({
            key : textboxkey,
...
document.querySelector('.btnMusicMinimize').addEventListener('click', function(){
    ...
    $.post('https://np-ui/np-ui:closeApp', JSON.stringify({}));
});
document.querySelector('.btnMusicClose').addEventListener('click', function(){
    widgetmusic.stop();
    $.post('https://np-ui/np-ui:closeApp', JSON.stringify({}),function (cb) { 
        $.post('https://np-ui/np-ui:applicationClosed', JSON.stringify({
            name : "musicplayer",
            fromEscape : false
        }),function(result){
...
window.addEventListener('message',function(event){
    ...
    switch (app) {
        case "memorygame": // Memmory Minigame
            if (showapp) {
                minigame_memory_endpoint = appdata.gameFinishedEndpoint ? appdata.gameFinishedEndpoint : "np-ui:heistsThermiteMinigameResult";
...
document.querySelector('.vol-up-wrapper').addEventListener('click', function(){
    $.post('https://np-ui/np-ui:radioVolumeUp', JSON.stringify({}));
});

document.querySelector('.vol-down-wrapper').addEventListener('click', function(){
    $.post('https://np-ui/np-ui:radioVolumeDown', JSON.stringify({}));
});

document.querySelector('.on-off-wrapper').addEventListener('click', function(){
    radiostate = !radiostate
    if (radiostate == true) {
        $.post('https://np-ui/np-ui:toggleRadioOn', JSON.stringify({}));
    }else if (radiostate == false) {
        $.post('https://np-ui/np-ui:toggleRadioOff', JSON.stringify({}));
    }
});

document.querySelector('#channelradio').addEventListener('change', function(e){
    if (!isNaN(e.target.value)) {
        $.post('https://np-ui/np-ui:setRadioChannel', JSON.stringify({
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        ...
        if (minigame_capthcha_active) {
            ...
            $.post('https://np-ui/np-ui:closeApp', JSON.stringify({}),function (cb) { 
                $.post('https://np-ui/'+minigame_captcha_endpoint, JSON.stringify({
