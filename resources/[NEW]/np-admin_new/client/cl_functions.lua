...
function ToggleDevMode(Bool)
    TriggerEvent('np-admin_new:client:ToggleDevmode')
    if Bool then
        while Bool do
            Wait(200)
...
function IsPlayerAdmin() 
    ...
    local IsAdmin = RPC.execute('np-admin_new:getPermission')
    if not IsAdmin then
         return 
    end
...
function GetPlayersInArea(Coords, Radius)
    ...
    local Coords, Radius = RPC.execute("np-admin_new:getActivePlayers")
...
function GetPlayers()
    ...
    local Players = RPC.execute("np-admin_new:getPlayers")
    if not Players then 
        return
    end
...
function GetBans()
    ...
    local Bans = RPC.execute("np-admin_new:getBans")
    if not Bans then 
        return
    end
