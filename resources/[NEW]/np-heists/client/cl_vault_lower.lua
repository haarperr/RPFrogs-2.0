...
local bicBoiVaultCoords = vector3(284.03118896484, 224.94183349609, 97.70467376709) 
...
RegisterNetEvent('np-heists:checkexplosion')
AddEventHandler('np-heists:checkexplosion', function()
  print('entering polyzone')
    Citizen.CreateThread(function()
        while inLower do
            Citizen.Wait(1)
            -- print(IsExplosionInArea(2, powerPanelCoords))
            -- print('ayo')
            local explosion = IsExplosionInRadius(bicBoiVaultCoords.x, bicBoiVaultCoords.y, bicBoiVaultCoords.z, 10)
            if explosion then
                -- print('hoe ass bitch ', explosion)
                print('hi')
                RPC.execute('np-heist:explodeVault')
                break
            end
        end
    end)
...
Citizen.CreateThread(function()
  ...
  exports['np-eye']:AddPeekEntryByPolyTarget('lower_vault_keypad', {{
...
AddEventHandler("np-heists:lowerVaultPanelPush", function(p1, p2, pArgs)
  ...
  if isPublic then
    ...
    exports["np-ui"]:openApplication("minigame-numbers", {
      gameFinishedEndpoint = publicGameFinishedEndpointNumbers,
...
RegisterUICallback(gameFinishedEndpointNumbers, function(data, cb)
  local codes = RPC.execute("np-heists:registerLVUsbUse")
  cb({ data = {}, meta = { ok = true, message = "done" } })
  if data.success then
    ...
    local information = {
      ["other"] = codes["kp_1"].code .. '-' .. codes["kp_2"].code .. '-' .. codes["kp_3"].code .. "-" .. codes["kp_4"].code,
  }
    TriggerEvent("player:receiveItem", "lvaccesscodes", 1, true, information)
  end
end)
