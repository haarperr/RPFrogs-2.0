...
function Login.nativeStart(AlreadySpawned)
	...
	TriggerEvent("np-hud:toggle", false)

	if not AlreadySpawned then
		tatCategory = exports["np-clothes"]:GetTatCategs()
		tattooHashList = exports["np-clothes"]:GetTatCategs()
		Login.custompeds = exports["np-clothes"]:GetCustomSkins()
...
function Login.NUICallback(data)
	...
	if data.action == "deleteCharacter" then
		...
		RPC.execute("np-char:deleteCharacter", data.actionData)
	...
	if data.action == "newCharacter" then
		...
		local created = RPC.execute("np-char:createCharacter", chardata)
