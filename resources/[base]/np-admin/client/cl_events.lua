...
RegisterNetEvent("Admin:Godmode", function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:toggleGodmode', Result['player'])
    end
end)
...
RegisterNetEvent('Admin:Teleport', function(Result)
    if IsPlayerAdmin() then
        ...
        TriggerServerEvent('np-admin:teleportPlayer', Result['player'], Result['type'])
    end
end)

RegisterNetEvent("Admin:Chat:Say", function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:chatSay', Result['message'])
    end
end)
...
RegisterNetEvent('Admin:Open:Clothing', function(Result)
    if IsPlayerAdmin() then
        ...
        TriggerServerEvent('np-admin:openClothing', Result['player'])
    end
end)
...
RegisterNetEvent('Admin:Open:Barber', function(Result)
    if IsPlayerAdmin() then
        ...
        TriggerServerEvent('np-admin:openBarber', Result['player'])
    end
end)
...
RegisterNetEvent('Admin:Open:Tattoo', function(Result)
    if IsPlayerAdmin() then
        ...
        TriggerServerEvent('np-admin:openTattoo', Result['player'])
    end
end)
...
RegisterNetEvent('Admin:Open:Outfits', function(Result)
    if IsPlayerAdmin() then
        ...
        TriggerServerEvent('np-admin:openOutfits', Result['player'])
    end
end)

RegisterNetEvent('Admin:Revive', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:reviveTarget', Result['player'])
    end
end)

RegisterNetEvent('Admin:Remove:Stress', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:removeStress', Result['player'])
    end
end)
...
RegisterNetEvent('Admin:Change:Model', function(Result)
    if IsPlayerAdmin() and Result['model'] ~= '' then
        ...
        if IsModelValid(Model) then
            TriggerServerEvent('np-admin:setModel', Result['player'], Model)
        end
    end
end)
...
RegisterNetEvent('Admin:Reset:Model', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:resetSkin', Result['player'])
    end
end)

RegisterNetEvent('Admin:Armor', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:setArmor', Result['player'])
    end
end)

RegisterNetEvent('Admin:Food:Drink', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:setFoodDrink', Result['player'])
    end
end)
...
RegisterNetEvent('Admin:Request:Job', function(Result)
    if IsPlayerAdmin() then
        if Result['job'] ~= '' then
            TriggerServerEvent('np-admin:requestJob', Result['player'], Result['job'])
        end
    end
end)
...
RegisterNetEvent("Admin:Drunk", function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:drunk', Result['player'])
    end
end)

RegisterNetEvent("Admin:Animal:Attack", function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:animalAttack', Result['player'])
    end
end)

RegisterNetEvent('Admin:Set:Fire', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:setFire', Result['player'])
    end
end)

RegisterNetEvent('Admin:Fling:Player', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:flingPlayer', Result['player'])
    end
end)

RegisterNetEvent('Admin:GiveItem', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:giveItem', Result['player'], Result['item'], Result['amount'])
    end
end)

RegisterNetEvent('Admin:Ban', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:banPlayer', Result['player'], Result['expire'], Result['reason'])
    end
end)

RegisterNetEvent('Admin:Kick', function(Result)
    if IsPlayerAdmin() then
        TriggerServerEvent('np-admin:kickPlayer', Result['player'], Result['reason'])
    end
end)
...
RegisterNetEvent('Admin:Toggle:PlayerBlips', function()
    ...
    TriggerServerEvent('np-admin:toggleBlips')
...
RegisterNetEvent('Admin:Toggle:Spectate', function(Result)
    ...
    if not isSpectateEnabled then
        TriggerServerEvent('np-admin:startSpectate', Result['player'])
    else
        toggleSpectate(storedTargetPed)
        preparePlayerForSpec(false)
        TriggerServerEvent('np-admin:stopSpectate')
    end
end)
...
RegisterNetEvent("np-admin:toggleGodmode", function()
...
RegisterNetEvent('np-admin:teleportPlayer', function(Coords)
    local Entity = PlayerPedId()    
    SetPedCoordsKeepVehicle(Entity, Coords.x, Coords.y, Coords.z)
end)

RegisterNetEvent('np-admin:setModel', function(Model)
    LoadModel(Model)
    SetPlayerModel(PlayerId(), Model)
    SetPedComponentVariation(PlayerPedId(), 0, 0, 0, 0)
end)

RegisterNetEvent('np-admin:armorUp', function()
...
RegisterNetEvent('np-admin:flingPlayer', function()
    local Ped = PlayerPedId()
...
RegisterNetEvent('np-admin:DeletePlayerBlips', function()
    if not IsPlayerAdmin() then return end
    DeletePlayerBlips()
end)

RegisterNetEvent('np-admin:UpdatePlayerBlips', function(BlipData)
    if not IsPlayerAdmin() then return end
...
RegisterNetEvent("np-admin:drunk", function()
    drunkThread()
end)

RegisterNetEvent("np-admin:animalAttack", function()
    startWildAttack()
end)

RegisterNetEvent("np-admin:setFire", function()
