...
function ToggleDevMode(Bool)
    TriggerEvent('np-admin:currentDevmode', Bool)
    if Bool then
        while Bool do
            Wait(200)
...
function IsPlayerAdmin() 
    ...
    local IsAdmin = RPC.execute('np-admin:getPermission')
    if not IsAdmin then
         return 
    end
...
function GetPlayersInArea(Coords, Radius)
    ...
    local Coords, Radius = RPC.execute("np-admin:getActivePlayers")
...
function GetPlayers()
    ...
    local Players = RPC.execute("np-admin:getPlayers")
    if not Players then 
        return
    end
...
function GetBans()
    ...
    local Bans = RPC.execute("np-admin:getBans")
    if not Bans then 
        return
    end
...
function GetAddonVehicles()
    local VehicleList = RPC.execute("np-admin:GetVehicleList")
