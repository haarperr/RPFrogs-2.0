...
function Caue.Admin.BeginAFK(self, stop)
    ...
    afk.event = AddEventHandler("np-adminold:afkStringCheck", function(text)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("np-adminold:Disconnect", "AFK Kick") return end
        end
    end)
end
...
RegisterNetEvent("np-adminold:afkStringCheck")

RegisterNetEvent("np-adminold:setStatus")
AddEventHandler("np-adminold:setStatus", function(src, status)
    local player = Caue._Admin.Players[src]
    if not player then return else Caue._Admin.Players[src].status = status end
end)

RegisterNetEvent("np-adminold:sendPlayerInfo")
AddEventHandler("np-adminold:sendPlayerInfo", function(data, discData)
    Caue._Admin.Players = data
    Caue._Admin.DiscPlayers = discData
end)

RegisterNetEvent("np-adminold:RemovePlayer")
AddEventHandler("np-adminold:RemovePlayer", function(src)
    local data = Caue._Admin.Players[src]
    Caue._Admin.DiscPlayers[src] = data
    Caue._Admin.Players[src] = nil
end)

RegisterNetEvent("np-adminold:AddPlayer")
AddEventHandler("np-adminold:AddPlayer", function(player)
...
function devMenu()
    ...
    if Caue.Admin:GetPlayerRank() == "dev" then
        TriggerEvent("np-adminold:openMenu")
    end
end
...
function devNoclip()
    ...
    if Caue.Admin:GetPlayerRank() == "dev" then
        ...
        TriggerEvent("np-adminold:NoclipState",bool)
...
function devCloak()
    if not devmodeToggle then return end
    if Caue.Admin:GetPlayerRank() == "dev" then
        TriggerEvent("np-adminold:CloakRemote")
    end
end
...
RegisterNetEvent("np-adminold:currentDevmode")
AddEventHandler("np-adminold:currentDevmode", function(devmode)
...
function Caue.Admin.RunCommand(self, args)
    ...
    TriggerServerEvent("np-adminold:runCommand", args)
end
...
function Caue.Admin.RunNclp(self,bool)
    ...
    TriggerEvent("np-adminold:noClipToggle", isInNoclip)
end
...
RegisterNetEvent("np-adminold:RunClCommand")
AddEventHandler("np-adminold:RunClCommand", function(cmd, args)
    Caue.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("np-adminold:updateData")
AddEventHandler("np-adminold:updateData", function(src, type, data)
...
RegisterNetEvent("np-adminold:noLongerAdmin")
AddEventHandler("np-adminold:noLongerAdmin", function()
    Caue._Admin.Players = {}
...
RegisterNetEvent("np-adminold:bringPlayer")
AddEventHandler("np-adminold:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("np-adminold:runSpawnCommand")
AddEventHandler("np-adminold:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
RegisterNetEvent('np-adminold:setLastVehicle')
AddEventHandler('np-adminold:setLastVehicle', function (pNetId)
...
RegisterNetEvent("np-adminold:SeatIntoLast")
AddEventHandler("np-adminold:SeatIntoLast", function()
    local rank = Caue.Admin:GetPlayerRank()
    local rankData = Caue.Admin:GetRankData(rank)
...
RegisterNetEvent("np-adminold:ReviveInDistance")
AddEventHandler("np-adminold:ReviveInDistance", function()
    local rank = Caue.Admin:GetPlayerRank()
    local rankData = Caue.Admin:GetRankData(rank)
...
RegisterNetEvent("np-adminold:cDistanceRevive")
AddEventHandler("np-adminold:cDistanceRevive", function()
