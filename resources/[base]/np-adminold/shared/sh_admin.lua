...
function Caue.Admin.IsValidUser(self, user)
    ...
    local steamid = getVar("steamid")
    local license = getVar("license")
    local src = getVar("source")
...
function Caue.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        for k,v in pairs(Caue._Admin.CurAdmins) do
            TriggerClientEvent("np-adminold:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("np-adminold:setStatus", status)
end
