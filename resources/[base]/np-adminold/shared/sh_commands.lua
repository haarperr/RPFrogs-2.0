...
function cmd.Init()
    ...
    RegisterNetEvent("np-adminold:NoclipState")
    AddEventHandler("np-adminold:NoclipState", function(toggle)
...
function cmd.RunCommand(caller, args)
    ...
    TriggerClientEvent("np-adminold:bringPlayer", targetSid, GetEntityCoords(GetPlayerPed(callerSid)))
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerClientEvent("np-adminold:Cloak", -1, src, args.toggle)
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then
        ...
        AddEventHandler("SpawnEventsServer", function(user, char)
            local src = source
            TriggerClientEvent("np-adminold:Cloak", src, cmd.vars.cloaked)
        end)

        AddEventHandler("playerDropped", function()
            local src = source
            if cmd.vars.cloaked[src] then
                TriggerClientEvent("np-adminold:Cloak", -1, src, false)
    ...
    RegisterNetEvent("np-adminold:CloakRemote")
    AddEventHandler("np-adminold:CloakRemote", function()
    ...
    RegisterNetEvent("np-adminold:Cloak")
    AddEventHandler("np-adminold:Cloak", function(player, toggle)
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerClientEvent("np-vehicles:spawnVehicle", src, args.message)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Seat into Vehicle") then TriggerEvent("np-adminold:SeatIntoLast") end
...
function cmd.RunCommand(caller, args)
    ...
    if args.time ~= nil and args.time ~= 0 then
        TriggerEvent("np-weathersync:setTime",src,args.time)
    end

    if args.weather ~= nil and args.weather ~= "" then
        TriggerEvent("np-weathersync:setWeather",src,args.weather)
    end
...
function cmd.Init()
    if IsDuplicityVersion() then
        ...
        RegisterNetEvent("np-adminold:RaveMode")
        AddEventHandler("np-adminold:RaveMode", function(enabled)
            cmd.vars.toggle = enabled
            TriggerClientEvent("np-adminold:RaveMode", -1, enabled)
        end)

        RegisterNetEvent("SpawnEventsServer")
        AddEventHandler("SpawnEventsServer", function()
            local src = source
            if cmd.vars.toggle then TriggerClientEvent("np-adminold:RaveMode", src, cmd.vars.toggle) end
        end)
    ...
    RegisterNetEvent("np-adminold:RaveMode")
    AddEventHandler("np-adminold:RaveMode", function(toggle)
        cmd.vars.toggle = toggle
...
function cmd.RunClCommand(args)
    TriggerServerEvent("np-adminold:RaveMode", cmd.vars.toggle)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then
        TriggerServerEvent("np-adminold:sDistanceRevive")
    end
...
function cmd.RunCommand(caller, args)
    ...
    local src = caller:getVar("source")
    TriggerEvent("np-jobs:changeJob", jobName, src)
end
