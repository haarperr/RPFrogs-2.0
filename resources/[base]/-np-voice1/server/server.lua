...
AddEventHandler("np:voice:transmission:state", function(pGroup, pContext, pTransmitting, pEffect)
	local src = source
	...
	if type(pGroup) == "table" then
		for k, v in pairs(pGroup) do
			TriggerClientEvent("np:voice:transmission:state", v, src, pContext, pTransmitting, pEffect, isSourceInVehicle)
		end
	else
		TriggerClientEvent("np:voice:transmission:state", pGroup, src, pContext, pTransmitting, pEffect, isSourceInVehicle)
	end
end)
