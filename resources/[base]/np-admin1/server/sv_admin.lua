...
local function addPlayer(src)
    ...
    for k, v in pairs(Caue._Admin.CurAdmins) do
        if k == src then
            TriggerClientEvent("np-admin:sendPlayerInfo", src, Caue._Admin.Players, Caue._Admin.DiscPlayers)
        else
            TriggerClientEvent("np-admin:AddPlayer", k, infos)
        end
    end
end
...
local function removePlayer(src)
    ...
    for k, v in pairs(Caue._Admin.CurAdmins) do
        TriggerClientEvent("np-admin:RemovePlayer", k, src)
    end
end
...
local function updatePlayer(src, var, value)
    ...
    for k, v in pairs(Caue._Admin.CurAdmins) do
        TriggerClientEvent("np-admin:updateData", k, src, var, value)
    end
end
...
AddEventHandler("np-admin:setStatus", function(status)
    local src = source
    ...
    for k, v in pairs(Caue._Admin.CurAdmins) do
        TriggerClientEvent("np-admin:setStatus", src, status)
    end
end)

RegisterNetEvent("np-admin:runCommand")
AddEventHandler("np-admin:runCommand", function(args)
    local src = source
    ...
    TriggerClientEvent("np-admin:RunClCommand",src,cmd,args)
end)
