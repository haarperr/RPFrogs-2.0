...
AddEventHandler('doTimer', function()
    ...
    TriggerServerEvent('np-death:dead', true)
    deathText = isMinor and ("Unconscious: ~r~ %d~w~ remaining seconds") or ("Dead: ~r~ %d ~w~ remaining seconds")
    respawnText = isMinor and ("~W~HOLD ~r~E ~w~ (%d) ~w~ TO ~r~GET UP~w~") or ("~w~HOLD ~r~E ~w~(%d) ~w~TO ~r~RESPAWN ~w~OR WAIT FOR ~r~EMS")
    local serverFarmText = ("~r~ DEAD: ~w~ Wait for the medical kit")
    local outbreakText = ("~w~ DEAD: ~r~F1 ~w~ TO SEE THE OPTIONS E ~r~RESPAWN ~w~ (~r~%d~w~ seconds)")
    while imDead == 1 do
        Citizen.Wait(0)
    ...
    inDoctorBed = false
    TriggerServerEvent('np-death:dead', false)
    TriggerEvent('pd:deathcheck')
end)
...
AddEventHandler('disableAllActions', function()
    if not disablingloop then
        ...
        while imDead == 1 do
            ped = PlayerPedId()
            Citizen.Wait(200)
            if isInVehicle() and (not IsDeadVehAnimPlaying() or IsPedRagdoll(ped)) and not FireCheck(ped) then
                DoVehDeathAnim(ped)
            elseif not isInVehicle() and (not IsDeadAnimPlaying() or IsPedRagdoll(ped)) and not FireCheck(ped) and not IsPedFalling(ped) then
                DoDeathAnim(ped)
            end
            Citizen.Wait(800)
        end
    ...
    while imDead == 1 do
        ...
        while thecount < 0 do
            ...
            if IsControlPressed(1,38) and (not IN_SERVER_FARM) then
                ...
                EHeld = EHeld - 1
                if hspDist > 5 and EHeld < 1 and not (isMinor and isInVehicle()) then
                    thecount = 99999999
...
AddEventHandler("trycpr", function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["np-taskbarskill"]:taskBarSkill(math.random(2000, 6000),math.random(5, 15))
        ...
        TriggerEvent("DoLongHudText","You are not near the resting place",2)
    end
end)
RegisterNetEvent("np-death:reset")
AddEventHandler("np-death:reset", function()
    thecount = 300
    imDead = 0
    isViolent = false
    isMinor = false
...
function releaseBody(dropItems)
    ...
    if dropItems then
        TriggerEvent("DoLongHudText","The EMS could not recover his belongings.",2)
        TriggerServerEvent("np-inventory:clear")
    end
    ...
    local doctors = RPC.execute('np-jobs:count', 'ems')
    if not doctors or doctors <= 0 then
        ...
        respawnText = ("Dead:Wait for a ~r~doctor~w~ to receive medical treatment or call a local doctor ~r~E (%d)")
    end
    local coords,hosp = getClosestHospital()
    SetEntityCoords(player, coords)
    Wait(2000)
    TriggerEvent("bed:checkin", doctors and doctors > 0)
    TriggerServerEvent('reviveGranted')
    ...
    Wait(15000)
    TriggerServerEvent("np-inventory:clear")
    TriggerEvent("DoLongHudText","EMS was unable to retrieve your belongings and you had amnesia.",2)
end
...
function attemptRevive()
    if not devmode and isInVehicle() then
