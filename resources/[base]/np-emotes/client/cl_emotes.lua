...
AddEventHandler('AnimSet:default', function()
    ResetPedMovementClipset(PlayerPedId(), 0)
    AnimSet = "default";
    TriggerServerEvent("np-emotes:setAnimData", AnimSet)
end)
...
AddEventHandler("Animation:Set:Gait", function(pArgs)
    ...
    if setGait == "move_m@swagger" then
      if exports["np-base"]:getChar("first_name") == "Ming" then
...
AddEventHandler("animation:PlayAnimation", function(anim)
    if not exports["np-base"]:getVar("dead") and not stuckincar then
...
AddEventHandler("expressions", function(pArgs)
    ...
    TriggerServerEvent("np-emotes:setExpData", expressionName)
