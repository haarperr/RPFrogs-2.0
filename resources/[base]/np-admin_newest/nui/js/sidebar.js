...
NPX.AdminMenu.LoadCategory = function(Category) {
    if (Category == 'Actions') {
        NPX.AdminMenu.LoadItems();
    } else if (Category == 'PlayerList') {
        NPX.AdminMenu.LoadPlayerList();
...
NPX.AdminMenu.SidebarAction = function(Action, Element) {
    if (Action == 'DevMode') {
...
$(document).on('click', ".menu-sidebar-action", function (e) {
    ...
    if (NPX.AdminMenu.Sidebar.Selected != OldSidebarCat && !Timeout) {
        Timeout = true;
        setTimeout(() => {
            Timeout = false;
        }, 300);
        if (NewSidebarCat.hasClass('lower')) {
            NPX.AdminMenu.SidebarAction(OldSidebarCat, NewSidebarCat)
        } else {
            let PreviousSidebarCat = $(`[data-Action="${NPX.AdminMenu.Sidebar.Selected}"]`);

            NPX.AdminMenu.LoadCategory(OldSidebarCat);
            // DebugMessage(`Changing Sidebar Category: ${NPX.AdminMenu.Sidebar.Selected} -> ${OldSidebarCat}`)
            
            $(PreviousSidebarCat).removeClass('selected');
            $(NewSidebarCat).addClass('selected');
    
            $(`[data-Page="${NPX.AdminMenu.Sidebar.Selected}"`).fadeOut(150);
            $(`[data-Page="${OldSidebarCat}"`).fadeIn(150);
    
            setTimeout(function(){ NPX.AdminMenu.Sidebar.Selected = OldSidebarCat; }, 200);
        }
    }
});