  ...
  for license, data in pairs(playerConnecting) do 
    ...
    if GetPlayerName(user) == nil or GetPlayerName(user) ~= name then 
      ...
      if Config.Debug then 
        print("RPFrogsQueue] (Thread : NO LONGER EXISTS) Popped player " .. name .. " from queue...");
      end
      ...
      if Config.Debug then 
        print("RPFrogsQueue] (Thread : NO LONGER EXISTS) currentConnectors is == " .. tostring(currentConnectors) )
      end
      ...
      if (GetPlayerCount() + 1) > slots or (GetPlayerCount() + Queue:GetMax()) > slots then  
        ...
        if not Queue:IsSetUp(user) then 
          ...
          if GetPlayerName(user) ~= nil then
            sendToDiscQueue("QUEUED USER", "Player `" .. GetPlayerName(user):gsub("`", "") .. "` has been added to the queue...", "RPFrogs Queue");
          end 
        ...
        if connecting[license] == nil or connecting[license] ~= true then 
          ...
          Wait(1000);
          if GetPlayerName(user) ~= nil then
            sendToDisc("NEW CONNECTOR", "Player `" .. GetPlayerName(user):gsub("`", "") .. "` is allowed to join now!", "RPFrogs Queue");
          end
          ...
          if Config.Debug then 
            print("RPFrogsQueue] currentConnectors is == " .. tostring(currentConnectors) )
          end
...
AddEventHandler('playerDropped', function (reason)
  ...
  if (Queue:IsSetUp(user)) then 
    Queue:Pop(user);
    sendToDiscQueue("REMOVED QUEUE USER", "Player `" .. GetPlayerName(user):gsub("`", "") .. "` has been removed from the queue...", "RPFrogs Queue");
...
AddEventHandler('DiscordQueue:Activated', function()
  ...
  if Config.Debug then 
    ...
    print("RPFrogsQueue] (DiscordQueue:Activated) currentConnectors is == " .. tostring(currentConnectors) )
  end
end)
