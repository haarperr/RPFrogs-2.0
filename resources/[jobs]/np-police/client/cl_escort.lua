...
function policeEscort()
	...
	if not inmenus and exports["np-jobs"]:getJob(job, "is_emergency") then
		local isInVeh = IsPedInAnyVehicle(PlayerPedId(), false)
		if isInVeh and exports["np-jobs"]:getJob(job, "is_police") then
			TriggerEvent("startSpeedo")
		else
			TriggerEvent("np-police:escort")
		end
	end
end
...
AddEventHandler("np-police:escort", function()
	if not isDisabled() and DecorGetInt(PlayerPedId(), "escorting") == 0 and DecorGetInt(PlayerPedId(), "escorted") == 0 and DecorGetInt(PlayerPedId(), "dragging") == 0 and DecorGetInt(PlayerPedId(), "dragged") == 0 and not IsPedRagdoll(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPlayerFreeAiming(PlayerId()) then
		...
		if distance ~= -1 and distance < 2.0 and GetEntitySpeed(ped) < 1.0 and not IsPedInAnyVehicle(ped, false) then
			if DecorExistOn(ped, "escorted") and DecorGetInt(ped, "escorted") == 0 then
				...
				TriggerServerEvent("np-police:escort", GetPlayerServerId(t), -1, false)
			end
		end
	else
		if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
			TriggerServerEvent("np-police:escort", DecorGetInt(PlayerPedId(), "escorting"), 0, false)
			DecorSetInt(PlayerPedId(), "escorting", 0)
		...
		if DecorGetInt(PlayerPedId(), "dragging") ~= 0 then
			TriggerServerEvent("np-police:escort", DecorGetInt(PlayerPedId(), "dragging"), 0, true)
...
RegisterNetEvent("np-police:escortReceive")
AddEventHandler("np-police:escortReceive", function(pToggle, pDrag)
	if pDrag then
...
RegisterNetEvent("np-police:escortingReceive")
AddEventHandler("np-police:escortingReceive", function(pToggle, pDrag)
	if pDrag then
...
RegisterNetEvent("np-police:unescort")
AddEventHandler("np-police:unescort", function()
	TriggerServerEvent("np-police:escorting", DecorGetInt(PlayerPedId(), "escorted"), 0, false)
	TriggerServerEvent("np-police:escorting", DecorGetInt(PlayerPedId(), "dragged"), 0, true)
...
RegisterNetEvent("np-police:drag")
AddEventHandler("np-police:drag", function()
	if not isDisabled() and DecorGetInt(PlayerPedId(), "escorting") == 0 and DecorGetInt(PlayerPedId(), "escorted") == 0 and DecorGetInt(PlayerPedId(), "dragging") == 0 and DecorGetInt(PlayerPedId(), "dragged") == 0 and not IsPedRagdoll(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPlayerFreeAiming(PlayerId()) then
		local t, distance, ped = GetClosestPlayer()
		...
		if distance ~= -1 and distance < 1.0 and GetEntitySpeed(ped) < 1.0 and not IsPedInAnyVehicle(ped, false) then
			if DecorExistOn(ped, "dragged") and DecorGetInt(ped, "dragged") == 0 then
				...
				TriggerServerEvent("np-police:escort", GetPlayerServerId(t), -1, true)
			end
		end
	else
		if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
...
RegisterNetEvent("np-police:resetEscort")
AddEventHandler("np-police:resetEscort", function()
	if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
		TriggerServerEvent("np-police:escort", DecorGetInt(PlayerPedId(), "escorting"), 0, false)
		DecorSetInt(PlayerPedId(), "escorting", 0)
		ClearPedTasksImmediately(PlayerPedId())
...
AddEventHandler("police:forceEnter", function()
	...
	if isInVeh then
		...
		if not AreAnyVehicleSeatsFree(veh) then
			TriggerEvent("DoLongHudText", "no seat available", 2)
			return
		end
...
AddEventHandler("police:forceSeatPlayer", function(vehNet, seat)
	...
	TriggerEvent("np-police:unescort")
	Citizen.Wait(100)
...
AddEventHandler("unseatPlayer", function()
	...
	if seat then
		...
		if DecorGetInt(PlayerPedId(), "escorting") == 0 then
			...
			TriggerServerEvent("np-police:escort", tServerId, -1, false)
		end
	else
...
Citizen.CreateThread(function()
	...
	exports["np-keybinds"]:registerKeyMapping("", "Police", "Escort / Speedo", "+policeEscort", "-policeEscort", "LEFT")

    RegisterCommand("+policeSeat", policeSeat, false)
	RegisterCommand("-policeSeat", function() end, false)
	exports["np-keybinds"]:registerKeyMapping("", "Police", "Force into Vehicle", "+policeSeat", "-policeSeat", "RIGHT")
end)
