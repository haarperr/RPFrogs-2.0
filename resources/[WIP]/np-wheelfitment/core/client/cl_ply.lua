...
function SyncWheelFitment()
    --if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            ...
            -- local vin = exports["np-garages"]:NearVehicle("plate")
            -- TriggerServerEvent("np-wheelfitment_sv:saveWheelfitment", vin, currentFitmentsToSet)
            RPC.execute("np-wheelfitment_sv:saveWheelfitment", currentFitmentsToSet)
...
Citizen.CreateThread(function()
    ...
    exports["np-polyzone"]:AddBoxZone("wheel_zone_1", vector3(144.93, -3030.51, 7.04), 6.6, 3.6)
end)
            ...
            if IsPedInAnyVehicle(plyPed, false) then
                if not isMenuShowing then
                    ...
                    if isNear(plyPos, cfg_wheelFitmentPos, 2.0) then
                        ...
                        if job > 1 then 
                        exports['np-textui']:showInteraction('[E] - Adjust Wheel Fitment')
