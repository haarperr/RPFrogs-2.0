...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0, targetInvSlots = 40, shopId) {
    ...
    if (targetinvName.indexOf('Drop') > -1) {
        ...
        slotLimitTarget = 200;
        displayName = 'Binder';
    } else if (targetinvName.startsWith('bento_box_')) {
...
function UpdateSetWeights(secondaryName) {
    ...
    if (!$('#mainInvHeading').html() || $('#secondaryInvName').attr('title') !== TargetInventoryName) {
        let mainHeader = `
            <h1 id='mainInvName' class='inventory-name' title='${PlayerInventoryName}'>Pockets</h1>
