...
function hasEnoughOfItem(itemid, amount, shouldReturnText, checkQuality, metaInformation)
    ...
    if itemid == nil or itemid == 0 or amount == nil or amount == 0 then
        if shouldReturnText then
            TriggerEvent("DoLongHudText","You don't seem to have " .. itemid .. " on you",2)
        end
        return false
    end
    ...
    if playerVeh ~= 0 then
        ...
        if IsThisModelACar(model) or IsThisModelABike(model) or IsThisModelAQuadbike(model) then
            if IsEntityInAir(playerVeh) then
                Wait(1000)
                if IsEntityInAir(playerVeh) then
                    TriggerEvent("DoLongHudText", "Looks like you're flying.", 2)
                    return false
                end
            end
        end
    ...
    if not validWaterItem[itemID] and not isWeapon then
        if IsPedSwimming(player) then
            ...
            if #(targetCoords - plyCoords) > 1.3 then
                TriggerEvent("DoLongHudText", "Can't use this while swimming.", 2)
                return false
            end
        end

        if IsPedSwimmingUnderWater(player) then
            TriggerEvent("DoLongHudText", "You can't use this while submerged.", 2)
            return false
        end
    end
...
AddEventHandler("RunUseItem", function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if itemid == "-72657034" then -- parachute
    ...
    if (itemid == "weedq") then
        local finished = exports["np-taskbar"]:taskBar(1000,"Roll one up",false,false,playerVeh)
    ...
    if (itemid == "disabler") then
        local finished = exports["np-taskbar"]:taskBar(500,"Starting",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "heistpadyacht" or itemid == "pixellaptop") then
        local finished = exports["np-taskbar"]:taskBar(1000,"Connecting to the network",false,false,playerVeh)
        if (finished == 100) then
            Wait(200)
   ...
   if (itemid == "nitrous") then
    ...
    if (itemid == "lighter") then
        ...
        local finished = exports["np-taskbar"]:taskBar(2000,"Sparking it up",false,false,playerVeh)
    end

    if (itemid == "joint") then
        local finished = exports["np-taskbar"]:taskBar(2000,"Smoking",false,false,playerVeh)
        if (finished == 100) then
            Wait(200)
    ...
    if (itemid == "water" or itemid == "cola" or itemid == "vodka" or itemid == "beer" or itemid == "whiskey" or itemid == "coffee" or itemid == "slushy" or itemid == "frappuccino" or itemid == "latte" or itemid == "fruitslushy")  then
        TriggerEvent("animation:PlayAnimation","drink")
    ...
    if (itemid == "mshake" or itemid == "softdrink")  then
        TriggerEvent("animation:PlayAnimation","drink")
        local finished = exports["np-taskbar"]:taskBar(3000,"Hydrating",false,false,playerVeh)
        if (finished == 100) then
            TriggerEvent('np-hud:ChangeThirst', 30)
            TriggerEvent("np-hud:updateStress",false,40)
            remove = true
        end
    end

    if (itemid == "sandwich" or itemid == "muffin" or itemid == "chocolate" or itemid == "fries" or itemid == "fishtaco" or itemid == "taco" or itemid == "churro" or itemid == "hotdog" or itemid == "greencow" or itemid == "eggsbacon" or itemid == "donut" or itemid == "icecream" or itemid == "hamburger" or itemid == "chocobar") then
        TriggerEvent("animation:PlayAnimation","eat")
        local finished = exports["np-taskbar"]:taskBar(3000,"Eating",false,false,playerVeh)
        if (finished == 100) then
            TriggerEvent('np-hud:ChangeHunger', 20)
            remove = true
        end
    end

    if (itemid == "bleederburger" or itemid == "heartstopper" or itemid == "torpedo" or itemid == "meatfree" or itemid == "moneyshot") then
        TriggerEvent("animation:PlayAnimation","eat")
        local finished = exports["np-taskbar"]:taskBar(3000,"Eating",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "fishingrod" or itemid == "fishingrod2" or itemid == "fishingrod3" or itemid == "fishingrod4") then
    ...
    if (itemid == "1gcocaine") then
        ...
        TaskItem("anim@amb@nightclub@peds@", "missfbi3_party_snort_coke_b_male3", 49, 5000, "Starting to game", "hadcocaine", true,itemid,playerVeh)
    end
    ...
    if (itemid == "1gmeta") then
        ...
        TriggerEvent("Evidence:StateSet",2,1200)
        TriggerEvent("Evidence:StateSet",6,1200)
        TaskItem("switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop", 49, 5000, "Using Metafetamina", "hadcrack", true,itemid,playerVeh)
    end
