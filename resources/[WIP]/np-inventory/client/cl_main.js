...
let itemListWithTax = async () => {
    const [fetchTax] = await RPC.execute("np-financials:getTax", "Goods");
...
let hasWeaponsLicense = async () => {
    const [fetchWeaponsLicense] = await RPC.execute("np-licenses:hasLicense", "weapon");
...
function PopulateGui(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, targetInvWeight, targetInvSlots, shopId) {
    let cid = exports["np-base"].getChar("id");
...
function GiveItem(itemid, amount, generateInformation, nonStacking, itemdata, returnData = '{}') {
    ...
    if (slot != 0) {
        cid = exports["np-base"].getChar("id");
        emitNet(
...
on('inventory-open-target', async (information) => {
    ...
    if (canOpen === true) {
        ...
        if(targetinvName.indexOf("Shop") > -1) {
            const [fetchCash] = await RPC.execute("np-financials:getCash");
            ...
            setImmediate(async () => {
                ...
                let cop = false;
                let job = exports["np-base"].getChar("job")
                if (exports["np-jobs"].getJob(job, "is_police") == true || job == "doc") {
                    cop = true;
                    WeaponsLicense = true;
                }
...
on('inventory-open-container', async (inventoryId, slots, weight) => {
    ...
    const cid = exports["np-base"].getChar("id");
...
on('inventory-bind', (slot) => {
    if (exports["np-base"].getVar('dead')) {
        return;
    }
    let cid = exports["np-base"].getChar("id");
...
RegisterNetEvent('np-inventory:log');
on('np-inventory:log', (pLog) => {
    SendNuiMessage(
        JSON.stringify({
            response: 'log',
            log: pLog,
...
on('__cfx_nui:insert-item', async (data, cb) => {
    ...
    if (
        ...
        if (isNaN(targetItemId)) {
            ...
            emitNet("np-inventory:ammo", originInventory, targetInventory, originSlot, targetSlot, originItemId, targetItemId, originItemInfo, targetItemInfo)
        }
    }
    cb({});
});
