            ...
            switch (pItemID.toLowerCase()) {
                case "idcard":
                    if (pItemData.fake) {
                        ...
                        let gender = "Male"
                        if (exports["np-base"].getChar(pSource, "gender") === 1) {
                            gender = "Female"
                        }

                        returnInfo = JSON.stringify({
                            ["ID"]: pCid,
                            ["First Name"]: exports["np-base"].getChar(pSource, "first_name").replace(/[^\w\s]/gi, ''),
                            ["Surname"]: exports["np-base"].getChar(pSource, "last_name").replace(/[^\w\s]/gi, ''),
                            ["Sex"]: gender,
                            ["Date of Birth"]: exports["np-base"].getChar(pSource, "dob")
                        })
                ...
                case "pdbadge":
                    exports.oxmysql.query(`SELECT c.first_name, c.last_name, c.job, (CASE WHEN m.image IS NULL THEN "0" ELSE m.image END) AS image, gr.name AS rank_name, (CASE WHEN j.callsign IS NULL THEN "000" ELSE j.callsign END) AS callsign FROM characters c INNER JOIN groups_members gm ON (gm.cid = c.id AND gm.group = c.job) INNER JOIN groups_ranks gr ON (gr.group = c.job AND gr.rank = gm.rank) LEFT JOIN mdt_profiles m ON m.cid = c.id LEFT JOIN jobs_callsigns j ON (j.cid = c.id AND j.job = c.job) WHERE c.id = '${pCid}'`, [], function(result) {
                        returnInfo = JSON.stringify({
                            ["_hideKeys"]: ["image", "job"],
                            ["First Name"]: result[0].first_name.replace(/[^\w\s]/gi, ''),
...
onNet("onResourceStart", (resource) => {
    if (resource == GetCurrentResourceName()) {
        setTimeout(() => {
            emit("np-inventory:luaItemList", itemList)
        }, 5000);
    }
})
...
onNet("server-inventory-move", async (pCid, pData, pCoords) => {
    ...
    if (purchase) {
        ...
        exports["np-financials"].updateCash(src, "-", itemCosts)

        let baseprice = itemList[itemidsent].price
        exports["np-financials"].addTaxFromValue("Goods", baseprice)
...
onNet("server-inventory-stack", async (pCid, pData, pCoords) => {
    ...
    if (purchase) {
        ...
        exports["np-financials"].updateCash(src, "-", itemCosts)
        exports["np-financials"].updateBalance(accountId, "+", itemCosts)
        exports["np-financials"].transaction(accountId, accountId, baseprice, comment, cid, 7)
        exports["np-financials"].addTaxFromValue("Goods", baseprice)
...
onNet("server-remove-item", async (pCid, pItemID, pAmount, pOpenedInv) => {
    let src = exports["np-base"].getSidWithCid(pCid)
...
onNet("np-inventory:clear", async (pSource, pName) => {
    let src = source
