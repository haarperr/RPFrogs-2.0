...
AddEventHandler('hud:saveCurrentMeta', function()
	TriggerServerEvent("np-hud:updateData",GetEntityHealth(PlayerPedId()),GetPedArmour(PlayerPedId()),currentValues["thirst"],currentValues["hunger"],currentValues["armor"])
end)
...
Citizen.CreateThread(function()
    while true do
		...
		Citizen.Wait(300000)
    	TriggerServerEvent("np-hud:updateData",GetEntityHealth(PlayerPedId()),GetPedArmour(PlayerPedId()),currentValues["thirst"],currentValues["hunger"])
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            SendNUIMessage({
                ...
                dev = developer,
                dev2 = debug,
                nitro = exports["np-nitro"]:HowmuchNitro(),                
                casino = tiempo,
                inVehicle = inveh,
                cinturon = harnessDur,
...
RegisterCommand('ui-r', function()
    ...
    exports['np-interaction']:showInteraction("Restarting UI .")
    Wait(1000)
    exports['np-interaction']:showInteraction("Restarting UI ..")
    Wait(1000)
    exports['np-interaction']:showInteraction("Restarting UI ...")
    Wait(1000)
    exports['np-interaction']:showInteraction("UI Successfully Restarted")
    Wait(1000)
