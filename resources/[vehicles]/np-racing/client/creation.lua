...
function startRaceCreation(options)
    ...
    while raceName ~= nil and options.raceName == nil and RPC.execute("np-racing:isRaceNameTaken", raceName) == true do
    ...
    Citizen.CreateThread(function()
        ...
        while creating do
            local pos = GetEntityCoords(ped)
            local instructions = "[E] Add Point | [Shift+E] Remove | ⬆ Up/Down ⬇ | ⬅ Rotate ➡"
...
function finishRaceCreation()
    ...
    TriggerServerEvent("np-racing:recieveCreateData", raceName, raceType, raceThumbnail, checkpoints)
...
RegisterNetEvent("np-racing:cmd:racecreate")
AddEventHandler("np-racing:cmd:racecreate", function(options)
    if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
        startRaceCreation(options)
    end
end)

RegisterNetEvent("np-racing:cmd:racecreatedone")
AddEventHandler("np-racing:cmd:racecreatedone", function()
    finishRaceCreation()
end)

RegisterNetEvent("np-racing:cmd:racecreatecancel")
AddEventHandler("np-racing:cmd:racecreatecancel", function()
    cancelRaceCreation()
end)
