...
function joinRace(src, id, alias, cid, dontpay)
    ...
    if PendingRaces[id]["buyIn"] > 0 and not dontpay then
        ...
        local success, message = exports["np-financials"]:transaction(accountId, 0, PendingRaces[id]["buyIn"], "", cid, 1)
        if not success then
...
RPC.register("np-racing:createPendingRace", function(src, id, options)
    ...
    local alias = exports["np-base"]:getChar(src, "aliases")
    ...
    if options["buyIn"] > 0 then
        ...
        if options["buyIn"] > bank then
            TriggerClientEvent("DoLongHudText", src, "You don't have $" .. options["buyIn"] .. " in your account!")
            return
        end

        local success, message = exports["np-financials"]:transaction(accountId, 0, options["buyIn"], "", cid, 1)
        if not success then
            TriggerClientEvent("DoLongHudText", src, message)
            return
        end
...
RPC.register("np-racing:finishedRace", function(src, curRaceid, bestLapTime, totalTime)
    ...
    if not ActiveRaces[curRaceid]["positions"] then
        ...
        local success, message = exports["np-financials"]:transaction(0, accountId, ActiveRaces[curRaceid]["prize"], "", 0, 1)
        if not success then
            TriggerClientEvent("DoLongHudText", src, message)
        end
