...
local bennysLocations = {
    ...
    ["bennystuner"] = {
        pos = vector3(135.963, -3031.130, 7.255),
        heading = 0,
        access = "tuner"
    },
    ["bennysimport"] = {
...
local function saveVehicle()
    ...
    local vid = exports["np-vehicles"]:GetVehicleIdentifier(veh)
    if vid then
        RPC.execute("np-vehicles:updateVehicle", vid, "metadata", "modifications", json.encode(vehicleMods))
    end
end
...
function RepairVehicle()
    ...
    TriggerEvent("np-vehicles:randomDegredation", plyVeh, 10, 3)
end
...
function ExitBennys()
    ...
    exports["np-interaction"]:showInteraction("Bennys")
...
AddEventHandler("bennys:enter", function()
    ...
    if bennysAccess then
        if bennysAccess == "emergency" and exports["np-jobs"]:getJob(false, "is_emergency") then
            goto hasAccess
        elseif exports["np-groups"]:GroupRank(bennysAccess) > 0 then
            fullAccess = true
            goto hasAccess
        end
