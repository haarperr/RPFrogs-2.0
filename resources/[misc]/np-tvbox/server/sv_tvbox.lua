...
RegisterNetEvent("np-tv:add", function(url, object, coords, scale, offset)
    ...
    TriggerClientEvent("np-tv:update", -1, tosend)
end)

RegisterNetEvent("np-tv:fetch", function()
    local tosend = {}
    ...
    TriggerClientEvent("np-tv:update", source, tosend)
end)

RegisterNetEvent("np-tv:setvolume", function(id, volume)
    if volume >= 0 and volume <= 10 then
        if data[id] then

            data[id]["Volume"] = volume

            TriggerClientEvent("np-tv:updatevolume", -1, id, volume)

        end
    end
end)

RegisterNetEvent("np-tv:destroy", function(id)
    if data[id] then
        data[id] = nil
        TriggerClientEvent("np-tv:delete", -1, id)
    end
end)